...
async function appendData(auth, ssID, sheetName, values) {
  ...
  sheets.spreadsheets.values.append( // This appends data!!!!! There MAY (so I'm not 100% sure) be a another way that just overwrite data
    {
      spreadsheetId: ssID,
      range: `${sheetName}!A2`,
...
async function moveCompletedTasks(auth, spreadsheetId, sheetFrom, sheetFromName, sheetTo, sheetToName) {
  ...
  const tryThis = () => {
    readData(auth, spreadsheetId, sheetFromName, 'A2:E').then((value) => {completedChecklist = value; isPromisePending.pop();}).catch(tryThis());
  }
