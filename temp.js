...
authorization.then(
  ...
  authorization.then(
      (auth) => {
        // deleteSheetTab(auth, ssID, 2087531897);
        return getSheets(auth, ssID);  // TODO — Save sheetIDList locally on device!! (Save API calls (Check once everytime the app opens though))
      }
  ).then((a) => {sheetIDList = a; console.log(sheetIDList)}).catch(console.error);
  setTimeout(() => {
    authorization.then(
...
authorization.then(
    (auth) => {
      // deleteSheetTab(auth, ssID, 2087531897);
      return getSheets(auth, ssID);  // TODO — Save sheetIDList locally on device!! (Save API calls (Check once everytime the app opens))
    }
).then((a) => {sheetIDList = a; console.log(sheetIDList)}).catch(console.error);
setTimeout(() => {
  authorization.then(
